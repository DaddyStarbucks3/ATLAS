...
from dataclasses import dataclass from typing import List, Dict, Any import random
@dataclass class AnswerRecord: email_id: int subject: str user_answer: bool correct_answer: bool correct: bool indicators: List[str] explanation: str
class GonePhishinGame: def init(self, emails: List[Dict[str, Any]], num_questions: int = 10, seed: int | None = None): if not emails: raise ValueError("Email list is empty.") if num_questions <= 0: raise ValueError("num_questions must be > 0")
